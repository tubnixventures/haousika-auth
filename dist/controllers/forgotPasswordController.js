...
export async function forgotPasswordController(c) {
    try {
        let body;
        try {
        ...
        // Fetch user
        const result = await execute(`SELECT id, email, phone_number FROM users WHERE email = ? OR phone_number = ? LIMIT 1`, [identifier, identifier]);
