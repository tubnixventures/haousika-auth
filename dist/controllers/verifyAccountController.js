...
export async function verifyAccountController(c) {
    try {
        ...
        if ("token" in data) {
            try {
                ...
                await execute(`UPDATE users SET is_verified=true, updated_at=NOW() WHERE id=$1`, [payload.id]);
        ...
        if ("user_id" in data && "otp" in data) {
            try {
                ...
                await execute(`UPDATE users SET is_verified=true, updated_at=NOW() WHERE id=$1`, [data.user_id]);
