...
export async function signupController(c) {
    ...
    try {
        ...
        const existing = await client.execute(`SELECT id FROM users WHERE email = ? OR phone_number = ?`, [email, phone_number]);
       ...
       VALUES (?,?,?,?,?,?,?,?,?,false,?,?)`, [id, assignedRole, "{}", email, phone_number, password_hash, display_name, country, timezone, now, now]);
        // Create session + JWT
