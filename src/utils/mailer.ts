 ...
 */
function buildTemplate(title: string, bodyContent: string) {
  return `
 ...
 * Specialized helpers
 */
export async function sendSignupAlert(to: string, displayName: string) {
...
export async function sendUpgradeNotification(to: string, newRole: string) {
  const html = buildTemplate(
    "Account Upgrade",
    `<p>Your account role has been upgraded to <strong>${newRole}</strong>.</p>
     <p>If you did not request this change, please contact support immediately.</p>`
  );
  return sendMail(to, "Account Upgrade Notification", html);
}

export async function sendResetEmail(to: string, otp: string, resetLink: string) {
  const html = buildTemplate(
    "Password Reset",
